...
The Owner Console uses the **shared skeleton system** built for all three
Agent Bento panels (full write-up in `shop.md` §10):

- `assets/js/app.js` — the `SKEL` builders plus the per-page map
  **`OB_SKEL`** covering every owner page (`overview`, `licenses`, `plans`,
  `administrators`, `shops`, `branding`, `website-pages`, `blogs`,
  `contact-messages`, `connectx`, `zudo`, `truebill`, `vaultium`,
  `helpdesk`, `addons`, `factory-reset`). `ownerPage(p)` paints
  `skelFor(OB_SKEL, p)` into `#page` synchronously — before the
  `platform/overview` fetch that gates every page — so navigation feedback
  is instant.
- `assets/css/app.css` — shared `.sk*` rules (structure + 1.5s shimmer);
  `assets/css/owner.css` maps the `--sk-*` tokens onto the owner variables
  (`--ob-card/-inset/-line/-ring/-text`), so skeletons follow the owner
  light/dark themes automatically.

Shapes mirror the real bento metrics (20px cards, 4→2→1 KPI grid at 960px /
640px, 14px tables, 31/36px rows), so content swaps in without layout
shift. Sub-loaders are covered too: the Zudo/Health log tables
(`#zudoOwnerLogs`, `#healthOwnerLogs`), the ConnectX provider log, the Zudo
conversation modal (`.ob-zudolog`), and the HelpDesk conversation pane
(`#hdConvMsgs`).

State contract: loading → skeleton · loaded → real · empty → `obEmpty` ·
error → the existing "Could not load platform control" panel. The shimmer
is disabled under `prefers-reduced-motion: reduce`.

Verified in the shared suite: render 44/44, action 15/15, browser 10/10 —
including owner overview skeleton → KPIs, theme toggle with skeletons in
both themes, license-page nav cycle, and the Zudo log modal.

---

## 9. Round 2 refinements

- **Page titles removed** — `obHead()` now renders only the head *actions*
  (e.g. "+ Post license plan"); the page title lives exclusively in the
  topbar (`#obTopTitle`). Dead `.ob-head` rules were pruned;
  `.ob-head-actions` is a standalone right-aligned row.
- **Brand skeleton** — the sidebar brand shows `sk()` shimmer bars on
  refresh until `public/branding` returns the owner-set name (fallback to
  defaults on failure). HelpDesk's administrator list also shows a list
  skeleton until `platform/helpdesk` resolves.
- **Table hover** — 7% tint + hovered `td`s use `--ob-text` so text stays
