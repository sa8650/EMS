...
| File | Role |
|---|---|
| `assets/css/app.css` | Public website + shared document styles (all panel themes removed; see `shop.md`) |
...
| Store grid | `.adm-storegrid` | **3-up** card grid (`repeat(3,minmax(0,1fr))`) |
| Plan grid | `.adm-plans` | **3-up** license-card grid |
...
**store cards** in a 3-up grid (`repeat(3,minmax(0,1fr))` — three cards per
row, wrapping 2-up below 1120px and 1-up below 640px). Each card is a
compact row-style card: name + address, shop-code
...
KPIs (license status, expiry, shop allowance, purchases) → **plan cards** in a
3-up grid (three per row, 2-up below 1120px, 1-up below 640px), built by
...
| Width | Behaviour |
|---|---|
| ≥1121px | Full layout: 264px sidebar, 4-col KPI, **store cards 3-up**, **plan cards 3-up** |
| 961–1120px | KPI 2×2, store/plan cards 2-up, bento grid tightens |
| ≤960px | Sidebar collapses to a top strip: brand + horizontal nav + user chip; cards stay 2-up |
...
- **Chromium style audit** — computed-style assertions in light + dark:
  ...
  no overflow, store grid 3-up with all three cards in one row at 1440px
  and license grid 3-up; mobile (390px): single column, horizontal nav, no
  overflow.
...
The Administrator Panel uses the **shared skeleton system** built for all
three Agent Bento panels (see `shop.md` §10 for the full write-up):

- `assets/js/app.js` — `SKEL` builders + the per-page map **`ADM_SKEL`**
  (`stores`, `licenses`, `addons`, `profile`, `devices`, `helpdesk`) and
  `skelFor(ADM_SKEL, p)`, painted into `#page` synchronously by
  `adminPage(p)` before any `await`, so navigation feedback is instant.
- `assets/css/app.css` — shared `.sk*` structure and shimmer;
  `assets/css/admin.css` maps the `--sk-*` tokens onto the admin variables
  (`--adm-card/-inset/-line/-ring/-text`), so skeletons follow the admin
  light/dark themes automatically.

Page shapes mirror the real bento metrics: store cards use `SKEL.cards`
(the 3-per-row grid at desktop, 2 at 1120px, 1 at 640px — same breakpoints
as `.adm-storegrid`), KPI rows use `SKEL.kpis(4)`, tables use
`SKEL.table(rows, cols)` with the 31px/36px row rhythm, and the HelpDesk
chat uses `SKEL.msgs` + a composer bar while `api('helpdesk')` resolves.

State contract: loading → skeleton · loaded → real · empty → `admEmpty` ·
error → the existing "Could not load this page" panel. No skeleton element
survives any terminal state, and the shimmer respects
`prefers-reduced-motion`.

Verified in the shared suite: render 44/44, action 15/15, browser 10/10 —
including admin store-card skeleton → real cards with the 3/2/1 responsive
column check, and helpdesk message skeleton → conversation.

---

## 11 · Round 2 refinements

- **Page titles removed** — `admHead()` now renders only the head *actions*
  (e.g. "Manage shop capacity" / "+ Create store"); the page title lives
  exclusively in the topbar (`#admTopTitle`), matching the Shop Panel
  pattern. Dead `.adm-head` rules were pruned; `.adm-head-actions` is a
  standalone right-aligned row.
- **Brand skeleton** — the sidebar brand shows `sk()` shimmer bars on
  refresh until `public/branding` returns the owner-set name (fallback
  to defaults on failure).
- **Table hover** — 7% tint + hovered `td`s use `--adm-text` so text stays
  readable during the highlight; `:active` gives click feedback.

Verified in the shared suite: render 51/51, action 15/15, browser 15/15.
